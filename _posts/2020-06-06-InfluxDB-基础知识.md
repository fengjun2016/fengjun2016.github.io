...
2.参数说明
* -d: 容器在后台运行
* -p: 将容器内端口映射到宿主机端口，格式为 宿主机端口:容器内部端口； 8033是influxDB的web管理工具端口，8086是influxdb的HTTP API端口
* --name: 容器名称 此处influxDBService则是启动后的容器名
* 若不加tag则启动的是最新版本latest

#### 客户端命令操作(基本语法)

> 这里使用docker安装方式的终端进行演示（连接influxDB实例之后 操作命令与其他地方是一样)

> 进入docker容器

```
docker exec -ti influxDbService /bin/bash
```

> 进入docker容器内部之后 连接influx实例

```
root@3f3d5b0f05be:/# influx
Connected to http://localhost:8086 version 1.8.0
InfluxDB shell version: 1.8.0
```

![显示](https://tva1.sinaimg.cn/large/007S8ZIlgy1gft1r9nltoj30f502wweu.jpg)

**相关参数说明** 
* influxDB 的http接口默认起8086上，所以influx默认也是连接的本地的8086端口
* -precision参数 表明了任何返回的时间戳的格式和精度，在上面的例子里，rfc3339是让InfluxDB返回RFC339格式(YYYY-MM-DDTHH:MM:SS.nnnnnnnnnZ)的时间戳
...
- **创建**
    ...
    ![](https://tva1.sinaimg.cn/large/007S8ZIlgy1gft1x73w0sj307z0673yo.jpg)
    
    * 这时候我们发现数据库有一个表"_internal",其实这个表是influxdb数据库的一些指标存储库。有点类似mysql数据库的mysql库。
    
- **删除**
    drop database {name};
    ![](https://tva1.sinaimg.cn/large/007S8ZIlgy1gft21dhughj307705ct8t.jpg)

- **使用**
    use {db_name};
    ![](https://tva1.sinaimg.cn/large/007S8ZIlgy1gft24ct0waj307m01pt8n.jpg)

#### **2.数据表和数据操作**

建库的操作可以发现非常类似于MYSQL下的操作。而在InfluxDB下没有细分的表的概念，InfluxDB下的  表在插入数据库的时候自动回创建。可以通过showmeasuresments命令查看所有的表，这个类似于MYS  QL下的show tables

- **显示所有表**

    show measurements
    ![](https://tva1.sinaimg.cn/large/007S8ZIlgy1gft290z1ofj306s04lwej.jpg)
    
- **新建表(写数据)**
    + 这里没有什么单独的创建表的语句，而是根据插入记录创建新表
    + 语法格式
        ```
